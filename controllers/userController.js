...
const loginUser = handleAsync(async (req, res) => {
  ...
  if (email) {
    user = await User.findOne({
      ...
      "+password -__v -createdAt -updatedAt -bankDetails -phoneNo -isP2P"
    );
  } else if (phoneNo) {
    user = await User.findOne({
      phoneNo: { $regex: phoneNo, $options: "i" },
...
const buySubscription = handleAsync(async (req, res) => {
  const user = req.user;
...
const getMyProfile = handleAsync(async (req, res) => {
  const user = await User.findById(req.user._id).select(
    "name email phoneNo walletAddress isSubscribed"
  );
